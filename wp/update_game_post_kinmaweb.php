...
// $pattern = "/プロ麻雀リーグ「(大和証券|朝日新聞) *Mリーグ(\d{4})(-\d{2,4})? *(セミファイナル)?」の(開幕|最終)?(第)?(\d{1,2})日目、第(\d)戦/";
$pattern = "/プロ麻雀リーグ「(大和証券|朝日新聞) *Mリーグ.?(\d{4})(-\d{2,4})? *(セミファイナル|ファイナル)?」の(開幕|最終)?(第)?(\d{1,2})日目、第(\d)戦/";
$pattern_day_index = 7;
