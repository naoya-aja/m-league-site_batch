...
foreach ($gamesList as $key => $geme) {
	...
	if (strtotime($date) < strtotime('2021-04-10')) continue;
	// echo $date."\n";
...
foreach ($array_sort as $i => $point) {
	...
	$ranking_teams[] = array_pad($arr, 12, '');
}
...
$file = __DIR__ . '/semifinal_team_scores.json';
file_put_contents($file, $json);

$json = json_encode($data_members);
$file = __DIR__ . '/semifinal_personal_scores.json';
file_put_contents($file, $json);

$json = json_encode($key_members);
$file = __DIR__ . '/semifinal_members.json';
file_put_contents($file, $json);

$json = json_encode($ranking_members);
$file = __DIR__ . '/semifinal_ranking.json';
file_put_contents($file, $json);

$json = json_encode($ranking_teams);
$file = __DIR__ . '/semifinal_ranking_teams.json';
file_put_contents($file, $json);
...
$file = __DIR__ . '/2020-semifinal-tm.csv';
...
$file = __DIR__ . '/2020-semifinal-indiv.csv';
